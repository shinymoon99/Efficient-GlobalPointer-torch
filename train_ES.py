...
@Description: 实体抽取.
"""
from utils.data_loader import EntDataset, load_data,load_ESdata
...
train_cme_path = 'datasets/ICTES/ICTES_train.json'  #CMeEE 训练集
eval_cme_path = 'datasets/ICTES/ICTES_dev.json'  #CMeEE 测试集
...
ner_train = EntDataset(load_ESdata(train_cme_path), tokenizer=tokenizer)
ner_loader_train = DataLoader(ner_train , batch_size=BATCH_SIZE, collate_fn=ner_train.collate, shuffle=True, num_workers=16)
ner_evl = EntDataset(load_ESdata(eval_cme_path), tokenizer=tokenizer)
...
for eo in range(EPOCH):
    ...
    with torch.no_grad():
        ...
        # if avg_f1 > max_f:
        if eo == EPOCH-1:
            torch.save(model.state_dict(), './outputs/ICTES/TEST_EP_L{}.pth'.format(eo))
            # max_f = avg_f1
        model.train()
